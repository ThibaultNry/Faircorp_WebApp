<template>
  ...
  >
    <div class="top-row d-flex" @click="toggleExpand">
      ...
      <div class="room-name text-muted">(ID: {{ window.id }})</div>
      ...
      >
        ...
        <template v-else> <span class="icon">&#x2716;</span> Closed </template>
      </div>
  ...
  methods: {
    ...
    async deleteWindow() {
      let response = await axios.delete(
        `${API_HOST}/api/windows/${this.window.id}/delete`,
        await this.sleep(1000),
        location.reload()
      );
      let deletedWindow = response.data;
      this.$emit("window-deleted", deletedWindow);
    },
  },
};
</script>
